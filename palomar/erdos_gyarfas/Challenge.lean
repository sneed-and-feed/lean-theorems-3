...
theorem disprove_erdos_gyarfas {V : Type u} [Fintype V] [DecidableEq V]
    (cert : CounterexampleCertificate V) : ¬ ErdosGyarfasConjecture.{u} := sorry
...
theorem K33_minDeg : ∀ v : Fin 6, K33.degree v ≥ 3 := sorry
...
theorem K33_cycle_length : K33_cycle.length = 4 := sorry

/-- The walk `K33_cycle` is a simple cycle. -/
theorem K33_cycle_isCycle : K33_cycle.IsCycle := sorry

/-- The length of `K33_cycle` is $4 = 2^2$, a power of two with exponent $\ge 2$. -/
theorem K33_cycle_power_of_two : IsPowerOfTwoCycleLength K33_cycle.length := sorry

/-- $K_{3,3}$ contains a power-of-two cycle. -/
theorem K33_hasPowerOfTwoCycle : HasPowerOfTwoCycle K33 := sorry
...
theorem K33_fails_counterexample_condition :
    ¬ (∀ (u : Fin 6) (p : K33.Walk u u), p.IsCycle → ¬ IsPowerOfTwoCycleLength p.length) := sorry
