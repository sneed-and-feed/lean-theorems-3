...
def main : IO UInt32 := do
  ...
  -- 1. Check all target declarations for type equality
  for thmName in thmNames do
    match envC.find? thmName with
    | none =>
      IO.eprintln s!"[FAIL] Declaration '{thmName}' not found in Challenge module!"
      hasMismatch := true
    | some infoC =>
      match envS.find? thmName with
      | none =>
        IO.eprintln s!"[FAIL] Declaration '{thmName}' not found in Solution module!"
        hasMismatch := true
      | some infoS =>
        if infoC.type == infoS.type then
          IO.println s!"[PASS] AST Expression Match: '{thmName}'"
        else
          IO.eprintln s!"[FAIL] AST Mismatch for '{thmName}':"
  ...
  -- 2. Transitive dependency constant verification
  IO.println s!"==> Checking {allUsed.size} transitive constant dependencies..."
    ...
    if !targetSet.contains c then
      ...
      | some infoC, some infoS =>
        ...
        if !typeEq || !valEq then
          IO.eprintln s!"[FAIL] Transitive Constant Mismatch for '{c}':"
          IO.eprintln s!"  typeMatch: {typeEq}, valMatch: {valEq}"
          hasMismatch := true
          constMismatchCount := constMismatchCount + 1
      | some _, none =>
        IO.eprintln s!"[FAIL] Transitive constant '{c}' missing in Solution!"
        hasMismatch := true
        constMismatchCount := constMismatchCount + 1
      | none, some _ =>
        IO.eprintln s!"[FAIL] Transitive constant '{c}' missing in Challenge!"
        hasMismatch := true
        constMismatchCount := constMismatchCount + 1
      | none, none => pure ()

  if hasMismatch then
    IO.eprintln s!"[FAIL] Constant equivalence verification failed with {constMismatchCount} mismatches."
    return 1
  else
